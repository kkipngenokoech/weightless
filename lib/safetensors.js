...
async function loadModel(fsPath) {
  ...
  // Adjacent training / auxiliary configs, if the checkpoint shipped them.
  // Tab label -> candidate filenames (first that parses as JSON wins).
  const SIDECARS = [
  ...
  for (const [label, cands] of SIDECARS) {
    for (const name of cands) {
      ...
      if (!fs.existsSync(p)) continue;
      try { extras[label] = { file: name, data: JSON.parse(await fs.promises.readFile(p, 'utf8')) }; break; }
      catch (_) { /* not JSON (e.g. training_args.bin pickle) — skip */ }
    }
  }

  return { tensors, fileSize, meta, config, extras, sharded: !!indexPath, shardFiles };
}
