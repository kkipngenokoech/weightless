...
/** Accept "owner/model", a full huggingface.co URL, or trailing junk. */
function normalizeId(input) {
  ...
  return parts.slice(0, 2).join('/'); // drop /tree/main, /blob/…, etc.
}

/** Load a Hub model's structure (same shape as safetensors.loadModel). */
async function loadFromHub(rawId, opts = {}) {
  ...
  const info = await fetchJson(`${BASE}/api/models/${id}?revision=${encodeURIComponent(revision)}`, token);
